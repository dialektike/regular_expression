...
여기서 빈줄을 찾으려면 다음과 같이 하면 됩니다. 줄바꿈이 연속적으로 2번이나 있다는 것은 빈줄이 있다는 것을 뜻하기 때문에 이렇게 하면 빈줄을 찾을 수 있습니다. 여기서 `rg`은 `--multiline` 옵션을 추가해새 적절하게 작동합니다.

- 정규 표현식: `\\`

```bash
rg '\n\n' ex/4_3.txt --multiline
```
...
```bash
rg '[[:xdigit:]][[:xdigit:]][[:xdigit:]][[:xdigit:]][[:xdigit:]][[:xdigit:]]' ex/3_5.html
```
